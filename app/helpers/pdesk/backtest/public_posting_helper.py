...
pdesk_pub_post_redis_prefix = "pdesk_pub_post" 


def public_posting_init():
    date_key = today_date_key()
    df_final_pdesk = get_cache(f"{pdesk_pub_post_redis_prefix}_init_{date_key}")
    if(isinstance(df_final_pdesk, pd.DataFrame) and len(df_final_pdesk)>0):
        return df_final_pdesk
    else:
        ...
        # df_monthly['month_date'] = pd.to_datetime(df_monthly['month_date'], format="%m/%Y")
        # df_monthly['legged_end_month_date_monthly'] = pd.to_datetime(df_monthly['month_date'], format="%m-%Y-%d") + MonthEnd(1)
        df_monthly['legged_start_month_date_monthly'] = pd.to_datetime(df_monthly['month_date'] + "/1", format="%m/%Y/%d")
        # print(df_monthly['legged_start_month_date_monthly'])
        del df_monthly['month_date']
        # del df_monthly['Date']

        tol = pd.Timedelta('30 day')
        df_final_pdesk = pd.merge_asof(left=df_daily.sort_values('Date'),right=df_monthly.sort_values('legged_start_month_date_monthly'), left_on="Date", right_on="legged_start_month_date_monthly", direction='backward',tolerance=tol)
        # print(df_final_pdesk[['Date', 'legged_start_month_date_monthly']].head(490).tail(50))
        ...
        set_cache(f"{pdesk_pub_post_redis_prefix}_init_{date_key}",df_final_pdesk,expiry_time=settings.time_to_expire)
        return df_final_pdesk
...
def function_for_startandend_monthandyear(y_variable):
    ...
    if y_variable == '2-3_soyoil_cbot_prices_USD_per_MT':
        for variable_2_3cbot in ['2_soyoil_cbot_prices_USD_per_MT', '3_soyoil_cbot_prices_USD_per_MT']:
            for file_name in glob.iglob(r"data/Pdesk/Public_posting/" + variable_2_3cbot + "/linear_regression/30-days-lookahead/*.csv", recursive=True):
...
def getting_publicposting_files_auto(y_variable, month, year,y_label,months_dict):
    ...
    # df_daily_for_renaming_2 = df_final_pdesk[['Date', '2-3_soyoil_cbot_prices_USD_per_MT']]
    # df_daily_for_renaming_3 = df_final_pdesk[['Date', '2-3_soyoil_cbot_prices_USD_per_MT']]
    ...
    if y_variable == '2-3_soyoil_cbot_prices_USD_per_MT':
        ...
        for variable_2_3cbot in ['2_soyoil_cbot_prices_USD_per_MT', '3_soyoil_cbot_prices_USD_per_MT']:
            for date in range(1, 32):  # this loop is for looping over the date
                try:
                    try:
                        if date <= 9:
                            date = str(0) + str(date)
                        temp = pd.read_csv(settings.DATA_FILE_PATH+"Pdesk/Public_posting/" + variable_2_3cbot + "/linear_regression/30-days-lookahead/"+
                        ...
                        #       "public-posting-pdesk-30-days-lookahead-" + str(date) + "_" + months[month] + "_" + str(
                        ...
                        temp["Date"] = pd.to_datetime(temp["Date"])  # changing the date to datetime
                        temp.sort_values(by=['Date'])
                        date_of_posting = str(2022) + "-" + months[month] + "-" + str(
                        ...
                        date_of_posting = temp["Date_of_posting"][0]  # selecting the date of posting
                        # print("date of posting------", date_of_posting)
                        ...
                        diff_shifted_and_unshifted_date = 1  # here setting the difference between shifted and  unshifted to 2
                        while diff_shifted_and_unshifted_date <= 4:  # here checking if the difference between shifted and  unshifted is less than equal to 4
                            if len(list(df_final_pdesk[df_final_pdesk['Date'] == unshifted_date][
                                ...
                                if len(list(df_final_pdesk[df_final_pdesk['Date'] == unshifted_date][[
                                    ...
                                    temp['y_unshifted'] = [value_of_y_unshifted for i in range(30)]
                                    diff_shifted_and_unshifted_date = 5  # for coming out of while loop
                                else:
                                    print()
                                diff_shifted_and_unshifted_date += 1  # if we dont have scappred data we will look for it again a day before
                    ...
                    if 'Soyoil-(CBOT)' in temp.columns:
                        temp.rename(columns={'Soyoil-(CBOT)': 'Soyoil(CBOT)_Prediction'}, inplace=True)

                    if variable_2_3cbot == '2_soyoil_cbot_prices_USD_per_MT':
                        # print(variable_2_3cbot)
                        mereged_df_2 = pd.merge(temp,
                                                df_daily_for_renaming_2[['Date', '2-3_soyoil_cbot_prices_USD_per_MT']],
                                                on='Date')
                        #                         print("&&&&&")
                        #                         print((df_final_pdesk[df_final_pdesk['Date'] == '2022-03-14'][['2-3_soyoil_cbot_prices_USD_per_MT']].values))
                        mereged_df_2.rename(columns={'2-3_soyoil_cbot_prices_USD_per_MT': 'Soyoil_shifted'},
                                            inplace=True)
                        stacked_mereged_df_2 = pd.concat([stacked_mereged_df_2, mereged_df_2], axis=0)
                        # print("for 2", len(stacked_mereged_df_2))
                        # print("staked data for 2")
                        # print(mereged_df_2)

                    if variable_2_3cbot == '3_soyoil_cbot_prices_USD_per_MT':
                        # print(variable_2_3cbot)
                        mereged_df_3 = pd.merge(temp,
                                                df_daily_for_renaming_3[['Date', '2-3_soyoil_cbot_prices_USD_per_MT']],
                                                on='Date')
                        mereged_df_3.rename(columns={'2-3_soyoil_cbot_prices_USD_per_MT': 'Soyoil_shifted'},
                                            inplace=True)
                        stacked_mereged_df_3 = pd.concat([stacked_mereged_df_3, mereged_df_3], axis=0)
                        # print('for 3', len(stacked_mereged_df_3))
                        # print("staked data for 3")
                        # print(mereged_df_3)
            ...
            if len(stacked_mereged_df_2) > 0 and len(stacked_mereged_df_3) > 0:
                blank_df = pd.concat([blank_df, stacked_mereged_df_2, stacked_mereged_df_3], axis=0)
        ...
        for date in range(1, 32):  # this loop is for looping over the date
            try:
                try:
                    if date <= 9:
                        date = str(0) + str(date)
                    temp = pd.read_csv(settings.DATA_FILE_PATH+"/Pdesk/Public_posting/" + y_variable + "/linear_regression/30-days-lookahead/"+
                                                   "public-posting-pdesk-30-days-lookahead-"
                                   + str(date) + "_" +
                                   months[month] + "_"+
                                   str(year)+ "-" +
                    ...
                    date_of_posting = str(2022) + "-" + months[month] + "-" + str(
                        date)  # creating date of posting to use it in getting the unshifted date
                    # print("date of posting", date_of_posting)
                    temp["Date_of_posting"] = [date_of_posting for i in range(30)]  # adding the date of posting column
                    temp["Date_of_posting"] = pd.to_datetime(
                ...
                if '3-CPO-(BMD)' in mereged_df.columns:  # checked for date 24-02-2022 3-CPO-(BMD) 3-CPO(BMD)_Prediction($/MT), y_variable - 3_cpo_bmd_prices_USD_per_MT
                    mereged_df.rename(columns={'3-CPO-(BMD)': '3-CPO(BMD)_Prediction($/MT)'}, inplace=True)

                if 'BMD' in mereged_df.columns:
                    mereged_df.rename(columns={'BMD': 'BMD_Prediction'}, inplace=True)

                if 'MCX' in mereged_df.columns:
                    mereged_df.rename(columns={'MCX': 'MCX_Prediction'}, inplace=True)

                if 'Soyoil-(NCDEX)' in mereged_df.columns:
                    mereged_df.rename(columns={'Soyoil-(NCDEX)': 'Soyoil(NCDEX)_Prediction'}, inplace=True)
...
def get_directional_correctness_graph(y_variable,y_label, month_case, year_case, high_impact_cases,months_dict,y_code):
...
def get_absolute_delta_graph(y_variable, month_case_abs, year_case_abs, y_label, months_dict,y_code):
