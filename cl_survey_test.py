...
def Provide_recommendation():
    ...
    for i in range(len(keys)):
        ...
        if float(cache_fulls) > 0:
            logfile.critical("The bandwidth configuration is not suffient for volume group {} , Try increasing the bandwidth 2 times".format(keys[i]))
            Run_Msg.update({'Bandwidth Recommendation':"The bandwidth configuration is not suffient for volume group {} , Try increasing the bandwidth 2 times.".format(keys[i])})
            ...
            logfile.info("The bandwidth configuration is suffient for volume group {}. ".format(keys[i]))
            Run_Msg.update({'bandwidth_recommendation':"The bandwidth configuration is suffient for volume group {} .".format(keys[i])})
        ...
        if ((total_local_writes/2)  > (total_remote_writes * 1024) + (float(size) * 1024 * 1024)) and (float(value)/1024 >  condition_2) :
            ...
            logfile.info("The cache configuration is suffient for volume group volume group {} ".format(keys[i]))
            Run_Msg.update({'cache_recommendation':"The cache configuration is suffient for volume group volume group {} ".format(keys[i])})
...
def terminate(err):
    ...
    if response==True:
        ...
        Run_Info={"run_duration":total_time,"interval":time_interval}
